...
FOR SELECT USING (get_user_role(auth.uid()) = ANY (ARRAY['admin'::app_role, 'security_officer'::app_role, 'staff'::app_role]));

CREATE POLICY "Admins and security can manage visitors" ON public.visitors
FOR ALL USING (get_user_role(auth.uid()) = ANY (ARRAY['admin'::app_role, 'security_officer'::app_role]));
